...
| Version | 3.3 — **`silver` moves from Fabric Lakehouse to Fabric Warehouse**, driven by dbt tooling maturity, not data-shape (§21-17). `bronze` remains on Fabric Lakehouse as a read-only dbt source. `gold` unaffected (already Warehouse); `recon` unaffected (SQL database in Fabric). See change note below. |
| Date | 10 August 2026 |
| Supersedes | v3.2 |
...
> **Change note (v3.3).** `silver` moves from Fabric Lakehouse to **Fabric Warehouse**. This is a tooling-driven change, not a reversal of the original data-shape reasoning (D2): the team's dbt project needs to write into `silver`, and the Lakehouse-side dbt adapter (`dbt-fabricspark`, Spark/Livy-based) does not yet support service-principal authentication for automated runs — a direct conflict with the "managed identity / service principal everywhere" security posture (§20.2). The Warehouse-side adapter (`dbt-fabric`, native T-SQL) supports service principal authentication today. This matches documented current practice for dbt-on-Fabric projects generally: Lakehouse as a read-only source layer, Warehouse as the dbt-managed transformation target. `bronze` is unaffected — it's populated by the Extraction Service, not dbt, so the adapter gap never applied to it, and it remains on Lakehouse as `silver`'s (now cross-item) source. See D24 (§3) and §21-17 for the open question this doesn't yet resolve — whether Fabric Warehouse's indexing model actually satisfies D8's sargable-predicate requirement, since Warehouse is a distributed query engine, not the same engine as `recon`'s SQL database in Fabric.

> **Change note (v3.2).** Two items closed out of design discussion, not build: (1) **SCD2 is adopted as the approach**, applied selectively rather than blanket — `dim_shop` and `vendor_master` are working candidates, not a confirmed final list, since the actual attribute volatility that justifies SCD2 can't be checked until NetSuite access is live. See D22 (§3), §10.4, and §21-16. (2) **Networking/access model for the AP Workspace is confirmed as the remaining go-live-blocking decision** — public access with a strong login layer (Front Door Standard + Entra ID/MFA) vs. Entra Private Access (fully private, per-user licensed) — see §21-15.
...
| D2 | **Three Fabric item types** — `bronze` as **Fabric Lakehouse** (Delta tables); `silver`/`gold` as **Fabric Warehouse**; **`recon` as SQL database in Fabric** *(Revised v3.3 — silver moved off Lakehouse; see D24)* | Each schema has one responsibility and now sits on the Fabric engine actually built for that responsibility. Lakehouse suits append-heavy, rebuildable, schema-flexible bronze. Warehouse suits gold's big-scan reporting queries and, as of v3.3, is also silver's home for tooling reasons (D24). `recon` is a live transactional workload — concurrent edits, FK-dependent writes, financial state that must never be silently overwritten — which is exactly what SQL database in Fabric is for: it's the same engine as Azure SQL Database, provisioned as a Fabric item, mirroring automatically into OneLake so it's still "in the lake" for reporting purposes |
...
| D22 | **SCD2 adopted as the approach for shared conformed dimensions — final entity list confirmed once NetSuite access is live** *(New, v3.2)* | Reporting scope beyond the KPIs already named in §18 isn't fully known yet, so whether a future report needs "attribute as of transaction date" can't be ruled out — defaulting to SCD2 as the pattern is cheaper than retrofitting it later. But *which* tables actually need it is a separate, still-open question: applying it to every dimension regardless of whether its attributes ever change would create unnecessary history tables for no benefit (same reasoning as quarantine, D-adjacent principle borrowed from a comparable engagement's design decisions). `dim_shop` and `vendor_master` are the working candidates, not a final list — confirmed once NetSuite access lands and actual attribute volatility (does a shop's region/ownership ever really change? does a vendor record get renamed/re-categorized?) can be checked against real data rather than assumed. `dim_shop` is additionally shared with the dashboard workstream (D14) — confirm whether SCD2 is already implemented there before building a second, possibly-divergent version |
| D23 | **SCD2 history is stored in SQL database in Fabric — the same item already provisioned for `recon` — in a separate schema, not as Fabric Warehouse/Lakehouse objects** *(New, v3.2)* | SCD2 history is irreplaceable once lost, the same property `recon` already has (D3) — a Gold rebuild that overwrote history with no prior capture would destroy it permanently, since Silver only ever holds current state. SQL database in Fabric is the one Fabric item with real transactional `MERGE` support and no rebuild-on-schedule convention, and it already auto-mirrors into OneLake, so a fourth Fabric item is not needed (Principle #6, "reuse, don't rebuild"). Held in its own schema alongside — not inside — `recon`, since it tracks dimension history, not workflow/transactional state, and the two have different lifecycles and ownership |
| D24 | **`silver` moves from Fabric Lakehouse to Fabric Warehouse; `bronze` stays on Fabric Lakehouse as a read-only dbt source** *(New, v3.3)* | Driven by dbt tooling, not data shape. The dbt project needs to write `bronze` → `silver` transformations, and the Lakehouse-side adapter (`dbt-fabricspark`) does not currently support service-principal authentication for automated runs — a direct conflict with the managed-identity-everywhere security posture (§20.2). The Warehouse-side adapter (`dbt-fabric`) supports service-principal auth today, and this Lakehouse-as-source / Warehouse-as-target split is documented current practice for dbt-on-Fabric projects generally. `bronze` itself is unaffected since it's written by the Extraction Service, not dbt — the adapter gap never applied to it. Whether Warehouse's indexing model fully satisfies D8's original sargable-predicate requirement is not yet confirmed — see §21-17 |
    ...
    no trigger)     │    accumulate        │  no processing yet
                    ...
                    └──────┬──────────┬───┘
                     ...
                     valid │          │ invalid / failed
                           ...
                           │   for Investigation
                    ┌──────▼──────────────┐
                    │  Silver Layer        │  Fabric Warehouse (silver)
...
**Architecture note (v3.3):** The diagram reflects the current platform split: **`bronze` on Fabric Lakehouse, `silver`/`gold` on Fabric Warehouse, `recon` on SQL database in Fabric** (see D1, D2, D24). All are Fabric/OneLake items — the client's "everything in Fabric OneLake" expectation is met — but `recon` sits on the one Fabric item type built for live transactional workloads, so the transactional-integrity concerns originally raised in D3/D21 (and re-flagged in v3.0's §21-14) are resolved by the engine itself, not worked around in application code. `silver`'s move to Warehouse (v3.3) is a dbt-tooling decision (D24), not a data-shape one — see §21-17 for the open question it doesn't yet close.
...
### 10.4 Point-in-time history for shared dimensions (SCD2) *(New, v3.2 — D22)*

SCD2 (effective/expiry dating, current-row flag) is adopted as the approach for shared conformed dimensions, applied **selectively** — not to every dimension by default:

- **Working candidates:** `dim_shop`, `vendor_master` — attributes that can plausibly change over time (region, ownership, GM, active status, per-vendor regex/tolerance configuration) where a future report might need the value as it stood at transaction time, not today's value.
- **Not a final list.** Whether SCD2 is actually worth building for a given table depends on whether its attributes change often enough in practice to matter — applying it everywhere regardless would create history tables with no real history to track. **Confirm the final entity list once NetSuite access is live** and actual attribute volatility can be checked against real data (does a shop's region/ownership genuinely change? does a vendor record get renamed or re-categorized?) rather than assumed up front.
- **Does not apply to:** transactional/decision history, which is already fully covered without SCD2 — `recon.audit_ledger`, the frozen `netsuite_snapshot`/`ccc_snapshot` per run (§16.4), and work item versioning (D20) already answer "what did this look like when the decision was made." SCD2 on a dimension is a different question ("what did the shop/vendor look like on that date") and doesn't duplicate or replace that machinery.
- **`dim_shop` ownership:** since this dimension is shared with the dashboard workstream (D14), confirm with that workstream whether SCD2 is already implemented there before building a second version — a divergent second `dim_shop` history would recreate the exact problem D14 exists to prevent.
...
### 16.5 SCD2 dimension history — storage location (D23, new v3.2)

**Distinct from everything above.** §16.1–16.4 describe `recon` — workflow and decision state. This section is about a different, separate concern: point-in-time history for shared conformed *dimensions* (D22), which answers "what did this shop/vendor look like on a given date," not "what reconciliation decision was made."

**Storage:** SQL database in Fabric — the same item already provisioned for `recon` — in its **own schema**, kept separate from `recon` because the two track fundamentally different things with different ownership and lifecycles:

```
SQL database in Fabric
 ├── recon.*     — workflow / transactional state (existing, §16.1–16.4)
 └── <history schema — name TBC>.*
       <one history table per SCD2-tracked entity — exact table names
        TBC alongside the final entity list, §21-16>
```

Each history table follows the standard SCD2 shape — business key, tracked attribute columns, `effective_start`, `effective_end`, `is_current` flag — but **exact schema name, table names, and column names are not decided here** and should not be assumed to match any name used elsewhere in this document; they are set when the final SCD2 entity list is confirmed (§21-16), most likely following whatever naming convention the dashboard workstream already uses for its own dimension tables, given the shared-ownership question already flagged in D22.

**Population:** a nightly job — extending the existing gold-rebuild pipeline (§18, Fabric Data Factory) rather than a new pipeline — compares current Silver dimension state against the current-flagged row in history. No tracked attribute changed → no write. A tracked attribute changed → expire the old row, insert a new current row. This is a standard MERGE, native to the SQL database in Fabric engine — no Delta-merge workaround needed, unlike Lakehouse.

**Gold's role stays unchanged from D11/§18.1** — Gold remains fully disposable and rebuildable. It does not store history itself; it reads the history schema (filtered to the current-flagged row for ordinary current-state reporting, unfiltered only for a report that specifically needs an as-of value) the same way it already reads `recon` for transactional facts.
...
### 19.1 Fabric OneLake configuration *(Revised v3.3)*
...
|---|---|---|
| `bronze` | **Lakehouse** (Delta tables) | Append-heavy, rebuildable-from-source, schema-flexible; matches the Delta table model natively. Read-only source for dbt (D24) — written by the Extraction Service, not by dbt, so the Lakehouse dbt adapter's auth limitation doesn't apply here |
| `silver` | **Fabric Warehouse** *(Moved from Lakehouse, v3.3 — D24)* | Tooling-driven, not data-shape-driven: this is where dbt writes the `bronze` → `silver` transformation, and the Warehouse-side dbt adapter (`dbt-fabric`) supports service-principal authentication today, unlike the Lakehouse-side adapter (`dbt-fabricspark`). Whether Warehouse's indexing model fully replicates the sargable-predicate guarantees the original Lakehouse-based reasoning assumed is **not yet confirmed** — see §21-17 |
...
| `gold` | **Fabric Warehouse** | T-SQL surface tuned for the big-scan reporting queries Power BI import mode runs against it. As of v3.3, shares an item with `silver` — a nightly gold-build no longer crosses Fabric items to reach silver |
...
-- Runs against silver (Fabric Warehouse, v3.3) via native T-SQL.
-- NOT YET CONFIRMED: whether Fabric Warehouse supports true CREATE INDEX
-- the way recon's SQL database in Fabric does, or relies on statistics/
-- partitioning like Lakehouse did — Warehouse is a distributed query
-- engine, a different engine from recon's OLTP database (§21-17, open).
-- Until confirmed, ensure vendor_id / invoice_normalized / amount drive
-- the join/filter predicate regardless, and partition silver.netsuite_bill
-- and silver.statement_line by vendor_id if Pass 1 join performance needs it.

-- recon (SQL database in Fabric) — standard, engine-enforced, confirmed:
-- One bill, one line
...
`invoice_normalized` must be a **persisted computed column (in `recon`/SQL database in Fabric) or a materialized column populated by the Normalization Service (in `silver`/Fabric Warehouse)** — never a function applied in the predicate, on either engine. Confirm Warehouse's actual indexing/statistics behavior for this column before assuming it performs the way `recon`'s does (§21-17).
...
| Matching service | Azure Container Apps | Reads `silver` (Fabric Warehouse, v3.3) only, via native T-SQL; writes to `recon` (SQL database in Fabric) via standard T-SQL/ADO.NET |
...
| Storage — `bronze` | **Fabric Lakehouse** | Delta tables; rebuildable, append-heavy; read-only dbt source (D24) |
...
| Storage — `silver`, `gold` | **Fabric Warehouse** *(silver moved here v3.3 — D24)* | T-SQL, dbt writes here for `silver`; tuned for reporting scans for `gold`, consumed by Power BI import mode. Indexing behavior for `silver`'s Pass 1 join key not yet confirmed — §21-17 |
...
| 15 | **Network access model for the AP Workspace.** Two options identified, both cost-viable — this is the sole remaining open architecture decision as of v3.2: **(A) Public + strong login** — Azure Front Door Standard ($35/mo base + custom WAF rules for geo-fencing/rate-limiting, included free) fronting the App Service, secured by Entra ID + mandatory MFA + Conditional Access + shop-level row-level security (§15.2); no per-location setup, scales cleanly across 79+ shops. **(B) Entra Private Access** — fully private, no public endpoint, agent-based zero-trust access; per-user licensed, cost scales with headcount rather than a flat infra fee. VPN Gateway and Application Gateway v2 were considered and ruled out on cost (flat ~$300+/month regardless of usage, no cheap tier, and per-location setup burden at 79 shops) | VIVE IT (Mark) | Networking / Security — must close before go-live |
| 16 | **Final SCD2 entity list** (D22, §10.4) — `dim_shop` and `vendor_master` are working candidates, not confirmed. Requires checking actual attribute volatility against real NetSuite data (does region/ownership/vendor categorization actually change?) rather than assuming it. Also requires confirming with the dashboard workstream whether `dim_shop` SCD2 is already built there (D14) | IF (internal) | Phase 5 — blocked on NetSuite access; not a go-live blocker |
| 17 | **Does Fabric Warehouse's indexing model actually satisfy D8's sargable-predicate requirement for `silver`?** (D24, §19.1, §19.2) — `silver` moved from Lakehouse to Warehouse for dbt-tooling reasons (service-principal auth), not because Warehouse's indexing was confirmed superior. Warehouse is a distributed query engine, not the same engine as `recon`'s SQL database in Fabric — need to confirm whether it supports true `CREATE INDEX`/seek behavior on `invoice_normalized` or relies on statistics/partitioning the way Lakehouse did, before assuming Pass 1 join performance is solved by the move alone. Also worth revisiting once Microsoft's native (non-Spark) Fabric Lakehouse dbt adapter reaches GA (~Q2 2026 per Microsoft's roadmap) — that may remove the original reason for this move | IF (internal) | Phase 2/6 — before Pass 1 matching performance testing |
