...
- **Matching engine**: 2-level deterministic hierarchy (Level 1: exact invoice number match; Level 3: RO number + amount match), configurable tolerances in `config/matching/matching_rules.json`, **zero AI involvement by design** — this is a hard invariant, not a limitation
