 ...
 * extractor, reusing scripts/extract_quirk.py (ported from the reference
 * implementation, see that file's own doc comment). */

const PYTHON_EXECUTABLE = process.env.PYTHON_EXECUTABLE ?? 'python';

export const QUIRK_SIGNATURES = ['QUIRK AUTO GROUP'];
export const QUIRK_VENDOR_SLUG = 'quirk_auto_group';
...
function runSubprocess(pdfPath: string): Promise<QuirkResult | { error: string }> {
  return new Promise((resolve, reject) => {
    const scriptPath = path.resolve(process.cwd(), 'scripts', 'extract_quirk.py');
    ...
    child.on('close', () => {
      try {
        ...
        reject(new Error(`extract_quirk.py produced non-JSON output: ${stdout || stderr}`));
      }
    });
  });
}

export async function extractViaQuirk(pdfBytes: Buffer): Promise<ExtractionOutcome> {
  const tmpPath = path.join(os.tmpdir(), `extract-quirk-${process.pid}-${Date.now()}.pdf`);
