...
export function computeDocumentStatus(documentId: string): DocumentStatusResult {
  ...
  // "Reconciled" — a match exists for one of this document's statement
  // lines. Checked first and unconditionally: IC-2/G2 guarantee a match can
  // only exist for a document whose latest extraction already passed
  // validation, so a match row is the authoritative terminal signal —
  // trusting that upstream guarantee here, not re-deriving it from attempt
  // history. No task before Session 5 ever writes to these tables; this
  // branch is forward-compatible, not exercised by a live pipeline yet.
  const matchRow = db
    .prepare(
