 ...
 * extractor, reusing scripts/extract_empire.py (ported from the reference
 * implementation, see that file's own doc comment). */

const PYTHON_EXECUTABLE = process.env.PYTHON_EXECUTABLE ?? 'python';

export const EMPIRE_SIGNATURES = ['EMPIRE AUTO PARTS'];
export const EMPIRE_VENDOR_SLUG = 'empire_auto_parts';
...
function runSubprocess(pdfPath: string): Promise<EmpireResult | { error: string }> {
  return new Promise((resolve, reject) => {
    const scriptPath = path.resolve(process.cwd(), 'scripts', 'extract_empire.py');
    ...
    child.on('close', () => {
      try {
        ...
        reject(new Error(`extract_empire.py produced non-JSON output: ${stdout || stderr}`));
      }
    });
  });
}

export async function extractViaEmpire(pdfBytes: Buffer): Promise<ExtractionOutcome> {
  const tmpPath = path.join(os.tmpdir(), `extract-empire-${process.pid}-${Date.now()}.pdf`);
