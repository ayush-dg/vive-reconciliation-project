...
Some statements also include an entire ROW that is not a transaction at all — e.g. "Previous Balance", "Balance Forward", "Opening Balance", or "Beginning Balance" — restating the amount carried over from a prior statement rather than a new invoice/repair-order charge or credit. Never extract such a row as a line item: it has no invoice or repair-order number of its own, and its amount is already reflected in the statement's own stated total, so including it as an extra line would double-count it. Only extract rows that represent an actual dated invoice, repair-order charge, or credit/payment transaction.
