 ...
 * extractor, reusing scripts/extract_astech.py (ported from the reference
 * implementation, see that file's own doc comment). Claude already
 * extracts this vendor correctly via the generic path — this port is for
 * cost/reliability, not correctness. */

const PYTHON_EXECUTABLE = process.env.PYTHON_EXECUTABLE ?? 'python';

export const ASTECH_SIGNATURES = ['asTech', 'Repairify'];
export const ASTECH_VENDOR_SLUG = 'astech_repairify';
...
function runSubprocess(pdfPath: string): Promise<AstechResult | { error: string }> {
  return new Promise((resolve, reject) => {
    const scriptPath = path.resolve(process.cwd(), 'scripts', 'extract_astech.py');
    ...
    child.on('close', () => {
      try {
        ...
        reject(new Error(`extract_astech.py produced non-JSON output: ${stdout || stderr}`));
      }
    });
  });
}

export async function extractViaAstech(pdfBytes: Buffer): Promise<ExtractionOutcome> {
  const tmpPath = path.join(os.tmpdir(), `extract-astech-${process.pid}-${Date.now()}.pdf`);
