...
**Currently enforced:** PARTIAL — the *code* contract holds (`client_factory.get_ai_client()` always resolves to exactly one deterministic primary, `provider_chain[0]`); the *documentation* contract is broken in six separate locations (RULES.md, Implementation Context, `gemini_client.py`, `client_factory.py`'s own comments, `ocr_extractor.py`, `document_understanding_engine.py`, `04_generate_report.py`'s docstring), all naming a different provider than the one actually resolved.
...
**Rationale:** RULE-04's intent is a single, documented, understood extraction path. The mechanism itself is sound (never ambiguous which provider runs); what's broken is every human-readable description of which provider that is. **[RESOLVED — Stage 3, 2026-07-24, engineer sign-off (P1-S3-002)]:** engineer confirmed Claude Sonnet 4.6 is intentionally the current primary. Documentation sweep completed the same day across all six locations (RULES.md RULE-04, Implementation Context Section 3, `gemini_client.py`, `client_factory.py`'s comments, `ocr_extractor.py`, `04_generate_report.py`). `Currently enforced` above should now read YES in both code and documentation — updated retroactively; the PARTIAL rating reflects the state at the time this invariant was first cataloged.
