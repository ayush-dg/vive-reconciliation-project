...
**Known divergences:**
- **[STAGE-2-DIVERGENCE]** Six separate code/doc locations (RULES.md RULE-04, Implementation Context, `gemini_client.py`'s docstring, `client_factory.py`'s own inline comments, `ocr_extractor.py`, `document_understanding_engine.py`'s docstring, `04_generate_report.py`'s docstring) each name a *different* provider as primary — all stale relative to this being the code-confirmed, currently-resolved primary.
