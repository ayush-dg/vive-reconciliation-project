...
| M-015 | extraction.ts | `components/B05_extraction.md` | Extract trigger entry point, G5 lock | **[UPDATED 2026-09-06, SPRINT-001 BCE refresh, ENH-001 Task 2.1]** Previously: "No rollback of the `'processing'` status column if the pipeline throws — a document could get permanently stuck." Now fixed for an in-process JS exception — `triggerExtraction` resets status to `'registered'` on a recoverable Silver-normalization failure or an exhausted-recovery error. Residual gap narrowed to a true OS-level process crash/restart only — see IC-CANDIDATE-01. |
...
| M-028 | aiProvider.ts | `components/C08_aiProvider.md` | Claude extraction, Azure Foundry/direct/mock routing | Fixed a max_tokens truncation bug (4096→16000) with a stop_reason guard. **[UPDATED 2026-09-06, SPRINT-001 BCE refresh, ENH-001 post-sign-off hotfix H1]** `EXTRACTION_SYSTEM_PROMPT` now explicitly excludes non-transactional summary rows ("Previous Balance", "Balance Forward", "Opening Balance", "Beginning Balance") from extracted lines — root-caused live against a real statement where such a row was extracted as a fake line item, failing both validation gates at once. NOT independently confirmed against a live-Claude retry of that exact case — the engineer's own retry resolved via the known-vendor deterministic route instead of Claude. |
...
| M-046 | api/documents/[id]/extract/route.ts | `components/U04_documents_extract_route.md` | POST extraction trigger, G5 lock | **[UPDATED 2026-09-06, SPRINT-001 BCE refresh, ENH-001 Task 2.1]** Previously: "Lock is non-releasing on failure — a pipeline exception mid-run leaves the document permanently stuck in `'processing'`, no `finally`/unlock path." Fixed for the in-process-exception case (see M-015) — residual gap narrowed to a true OS-level process crash/restart only. Also gained a third response: 422, when a document's Silver-recovery attempt is exhausted (see TOPOLOGY.md A01). |
...
| M-070 | UploadForm.tsx | `components/U23_UploadForm.md` | Upload flow, auto-chains extraction | Auto-chains a silent, un-awaited extraction call after upload — **[UPDATED 2026-09-06, SPRINT-001 BCE refresh, ENH-001 Tasks 2.2–2.4]** now true only for a SINGLE-file upload (unchanged, pre-ENH-001 behavior). A 2+-file batch (up to 15) instead awaits each file's full register+extract cycle sequentially — deliberately not fire-and-forget for multi-file batches. New per-file progress UI, table-wide click-through gate while any multi-file batch is non-terminal, single running success-counter toast for batches >1 file. New dependency: M-084. Fixed `LEGAL_ENTITIES[0]` as default entity (unchanged). |
...
| M-076 | DocumentDetailView.tsx | `components/U29_DocumentDetailView.md` | Document Detail rendering | Hard-coded `PROVIDER_LABELS` map silently falls back to raw provider strings for unmapped values. **[UPDATED 2026-09-06, SPRINT-001 BCE refresh, ENH-001 post-sign-off hotfix H2]** The standalone "Extraction summary" per-provider panel was removed; the same information is now folded into the existing `reconciliation-progress` sentence across all three of its states (no lines yet / not reconciled yet / complete). |
...
| M-084 | batchUploadSequencing.ts | `components/U37_batchUploadSequencing.md` | Sequences multi-file batch register+extract cycle | No fragility identified — pure function, no internal state; correctness reduces to its two injected callbacks behaving as documented (added 2026-09-06, SPRINT-001 BCE refresh, ENH-001 Task 2.2) |
