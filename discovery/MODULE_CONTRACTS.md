...
**1. Confidence fabrication was traced to exact line numbers across all four affected clients; M-023 (Claude Sonnet, the active primary) was fixed 2026-07-24.** `claude_sonnet_client.py` no longer hardcodes `ROW_CONFIDENCE = 0.75` — it now elicits and parses a genuine per-row confidence value (see `discovery/RISK_REGISTER.md` R-001). M-025 (`gemini_client.py:595`), M-026 (`mistral_client.py:356`), and M-024 (`document_intelligence_client.py:242`) still hardcode `ROW_CONFIDENCE = 0.75` — Gemini/Mistral remain broken but dormant; M-024 is also dormant. M-021 (Azure OpenAI), M-022 (Claude Haiku), and now M-023 preserve genuine model-elicited confidence; M-028 (`pdfplumber_fallback.py`) is the only path with real, differentiated confidence (0.65/0.50). Already fully documented in `discovery/components/A02_module_call_map.md` and `discovery/DOMAIN_MODEL.json` — this module-contract pass confirms the exact code location in each file rather than adding new severity.
