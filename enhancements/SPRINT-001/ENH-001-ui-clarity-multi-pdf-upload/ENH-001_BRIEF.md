...
| Toast notification behavior under batch upload (one popup per file vs. one consolidated summary) | **RESOLVED 2026-09-03** | Running success-only counter toast: "N/10 uploaded", updated by dismissing the previous toast and adding a new one on each file's completion (reuses existing `dismiss()`/`add()` in `toastStore.ts` — no new primitive, `M-083` stays untouched). UploadForm (M-070) tracks the current toast id across the batch loop. Failures are NOT rolled into this counter — shown per-row via existing per-file progress state instead, consistent with the sequential-extraction constraint above. |
