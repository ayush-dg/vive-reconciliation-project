...
| Toast notification behavior under batch upload (one popup per file vs. one consolidated summary) | **OPEN — not yet decided** | `ToastProvider`'s own contract has no cap on simultaneous toasts (Stage 2 finding) — a naive extension would mean up to N stacking popups for an N-file batch. Needs an explicit decision before this item is built, not assumed either way. |
