...
| Extraction pipeline — crash-recovery error typing | MODULE_CONTRACTS.md | M-022 (extractionPipeline.ts) — surfaced at the Phase 4 Design Gate (Finding 1): the IC-CANDIDATE-01 fix needs a distinguishable error type here, not just a change at the M-015 call site; see Known Constraints and `ENH-001_PHASE4_GATE_RECORD.md` |
...
| IC-CANDIDATE-01 (extraction lock has no crash-recovery path) — batch upload multiplies exposure | **MANDATORY** — resolved 2026-09-03, **redesigned at Phase 4 Design Gate** | **Decision: fix it, as part of this enhancement.** Original approach (generic try/finally around `extraction.ts:49`'s call site) was found at the Design Gate (Finding 1, `ENH-001_PHASE4_GATE_RECORD.md`) to target the wrong scenario — that call site's failure path is already caught internally by `runExtractionPipeline` and never actually throws there; the real gap is narrower, only when `normalizeToSilver()` fails after a successful extraction. **Confirmed approach (Option B):** a distinguishable `SilverNormalizationFailure` error type in `extractionPipeline.ts` (M-022), plus an opt-in `skipSuccessGuard` bypass of the existing success guard, wired through `extraction.ts`'s (M-015) trigger path for the recovery retry only — zero behavior change for any existing caller. No new column, no schema change, stays Tier 2 (this enhancement's tier, per `ENH-001_SCOPE.md`, was already Tier 2 for other reasons — multi-session — not because of this fix). (Alternative considered and rejected: a TTL-timestamp column mirroring the matching lock's staleness-reclaim — would have required a schema change.) |
