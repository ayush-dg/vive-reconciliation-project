...
| Uploaded Statements | `bronze.document` | "No statements uploaded yet" | TBD — polling vs. manual | Document Detail (`/documents/:id`, resolved 2026-08-26) |
...
| Section | Entity / Field | Source | Notes |
|---|---|---|---|
| Header | Vendor, period, status badge | `bronze.document` + Task 2.3's status computation | Same status badge values as Home |
| Extraction summary strip | Counts by extraction provider (`python_library_pdfplumber`, `claude_sonnet`, `pdfplumber_fallback`) | `bronze.extraction_attempt` (Task 3.5's summary endpoint) | pdfplumber_fallback labeled plainly as "via OCR fallback" for the AP user |
